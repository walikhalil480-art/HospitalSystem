...
export const getDashboardStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [
      ...
      prisma.staff.count({ where: { deletedAt: null, user: { role: "DOCTOR" } } }),
      prisma.staff.count({ where: { deletedAt: null, user: { role: "NURSE" } } }),
      ...
      prisma.medicalRecord.count({ where: { deletedAt: null, status: "OPEN" } }),
      prisma.medicalRecord.count({ where: { deletedAt: null, status: "CLOSED" } }),
      prisma.medicalRecord.findMany({
    ...
    const diagnosesByDepartment = medicalRecordAnalytics.reduce<Record<string, number>>((acc, record) => {
